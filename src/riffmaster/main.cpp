...
Detour XamInputGetStateDetour;           // XamInputGetState, ordinal 401
...
static RiffmasterState g_gipState;
...
static bool    g_gipAuthStarted = false;
...
static void GipFillGuitarCaps(BYTE& type, BYTE& subType, WORD& flags, XINPUT_GAMEPAD& pad) {
	...
	subType = GipSubTypeForCurrentTitle();   // 0x06 unless a verified override applies
	flags   = XINPUT_CAPS_VOICE_SUPPORTED | XINPUT_CAPS_PMD_SUPPORTED;   // 0x000C

	memset(&pad, 0, sizeof(pad));
	pad.wButtons = 0xF57F;                   // which buttons this device has
	pad.bLeftTrigger = 0xFF;                 // pickup switch rides the left trigger
	pad.bRightTrigger = 0x00;
	pad.sThumbLX = 0;                        // left stick unused on a 360 guitar
	pad.sThumbLY = 0;
	pad.sThumbRX = (SHORT)0xFFC0;            // whammy
	pad.sThumbRY = (SHORT)0xFFC0;            // tilt
}
	...
	while (off + 4 <= len) {
		...
		case GIP_CMD_IDENTIFY:
			...
			if (hdr.packetLength == 0 && !g_gipPoweredOn) {
				...
				// Start auth here rather than waiting for a connected STATUS.
				// The first attempt gated on STATUS + g_gipPoweredOn and never
				// fired: STATUS can arrive before the identify terminator that
				// sets that flag, so the condition was false when it mattered.
				// Driving it from the deterministic point in our own sequence
				// removes the ordering dependency entirely. The STATUS path below
				// remains as a fallback and is idempotent via g_gipAuthStarted.
				if (!g_gipAuthStarted) {
		...
		case GIP_CMD_VIRTUAL_KEY:
			if (hdr.packetLength >= 2 && payload[1] == GIP_VKEY_GUIDE) {
				g_gipState.guide = (payload[0] != 0);
				RM_DBG("RIFFMASTER: GIP GUIDE %s\r\n", payload[0] ? "DOWN" : "UP");
			}
			break;

		case GIP_CMD_INPUT:
			if (RiffmasterParseInput(payload, (int)hdr.packetLength, &g_gipState)) {
				g_gipInputsSeen++;
				// Rate-limited: these arrive at ~40 Hz and would flood the log.
				if (g_gipInputsSeen <= 3 || (g_gipInputsSeen % 400) == 0)
					RM_DBG("RIFFMASTER: GIP INPUT #%d btn=%04X up=%02X lo=%02X tilt=%02X whammy=%02X pickup=%02X\r\n",
						g_gipInputsSeen, g_gipState.buttons, g_gipState.upperFrets,
						g_gipState.lowerFrets, g_gipState.tilt, g_gipState.whammy,
						g_gipState.pickup);
			}
			break;
...
//
// ---- the two entry points hiddriver360 does NOT hook -------------------------
...
//   0x191 = 401  XamInputGetState             <- NOT hooked upstream
...
// at it using the plain non-Ex calls. Both are now covered.
//
...
DWORD XamInputGetStateHook(DWORD user, DWORD flags, XINPUT_STATE* state) {
	DWORD status = XamInputGetStateDetour
		.GetOriginal<decltype(&XamInputGetStateHook)>()(user, flags, state);
	...
	if (g_gipUserIndex != 0xFF && user == g_gipUserIndex && g_gipAuthStage == 7) {
		RiffmasterToXInput(&g_gipState, &state->Gamepad);
		state->dwPacketNumber = ++g_gipPacketNumber;
		return ERROR_SUCCESS;
	}
	return status;
}
...
BOOL APIENTRY DllMain(HANDLE Handle, DWORD Reason, PVOID Reserved) {
	if (Reason == DLL_PROCESS_ATTACH) {
		...
		if (XamInputGetStatePtr) {
			XamInputGetStateDetour = Detour(XamInputGetStatePtr, (void*)XamInputGetStateHook);
			XamInputGetStateDetour.Install();
			RM_LOG("RIFFMASTER: hooked XamInputGetState (401) @ %p\r\n", XamInputGetStatePtr);
		}
		else RM_LOG("RIFFMASTER: ordinal 401 did NOT resolve!\r\n");
